...
const SingleApplicant = () => {
  const {jobId, applicantId,hash } = useParams();

  const { getAllApplicants, allApplicants, convertToStandardDateTime,approveApplicant } =
  ...
  useEffect(()=>{
    if(approval && applicantId && jobId){
    const applicantApproval = async()=>{
    await approveApplicant(applicantId,jobId,approval);
    // setApproval("");
    navigate(`/auth/authority/${hash}/profile/job/${jobId}`)
  ...
  return (
   ...
   <div className='py-5 w-fit mb-5'>
    ...
    <Info label={"Deadline "} value={convertToStandardDateTime(singleApplicantData?.job?.deadline)} />
     ...
     <div className="mt-10 w-fit py-5 flex items-center justify-start gap-5">
        <span onClick={()=>setApproval("reject")} className="py-2 rounded px-6 bg-red-500 hover:bg-red-600 cursor-pointer text-lg">
            Reject Application
        </span>
        <span onClick={()=>setApproval("accept")} className="py-2 rounded px-6 bg-blue-500 hover:bg-blue-600 cursor-pointer text-lg">
            Accept Application
        </span>
