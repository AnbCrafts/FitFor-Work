...
const User = () => {
  ...
  const [currentSeekerPage, setCurrentSeekerPage] = useState(1);
  const seekersPerPage = 6;
  const indexOfLastSeeker = currentSeekerPage * seekersPerPage;
  const indexOfFirstSeeker = indexOfLastSeeker - seekersPerPage;
  const currentSeekers = allUsersList?.seekers.slice(
    indexOfFirstSeeker,
    indexOfLastSeeker
  );
  const totalSeekerPages = Math.ceil(
    allUsersList?.seekers.length / seekersPerPage
  );
  ...
  useEffect(()=>{
    if(singleUserId !== null){
      navigate(`single-seeker/detail/${singleUserId}`)
    }
  },[singleUserId])
  ...
  return (
    <div className="min-h-[100vh] w-[90%] mx-auto">
      ...
      {/* SEEKERS RENDER */}
      <div className="py-5 px-5 bg-gray-800 z-0">
        <h1 className="text-3xl mb-5">Job Seekers List</h1>

        {view === "List View" ? (
          <div>
            {currentSeekers?.map((item, index) => {
              return (
                <div
                  key={index}
                  className="py-1 bg-gray-900 flex items-center justify-between mb-2 text-center cursor-pointer hover:shadow-xl"
                  onClick={()=>setSingleUserId(item._id)}
               >
        ...
        <PageNav currentPage={currentSeekerPage} totalPages={totalSeekerPages} incrementer={setCurrentSeekerPage} />
      </div>
      </div>
